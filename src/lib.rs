...
pub async fn run(cfg: Config) {
    ...
    loop {
        ...
        let indexing_rate = match indexing_rate {
            Ok(rate) => {
                info!("INDEXING RATE: {} docs/s", rate);
                rate
            }
            Err(e) => {
                error!("Encountered error while fetching INDEXING RATE {}", e);
                f64::NAN
            }
        };

        let payload = json!({
            "ES_INFO": indexing_rate,
            "SYS_INFO": sysinfo,
            "@timestamp": sysinfo.last_updated
        });

        let response = post_metrics(payload, &cfg.server_url);
...
pub fn post_metrics(payload: Value, url: &str) -> Result<Response> {
    debug!("Trying to post payload:: {:#?}", payload);
    let resp = attohttpc::post(url).json(&payload)?.send()?;
    Ok(resp)
}
